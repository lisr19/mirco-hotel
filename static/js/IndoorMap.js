...
var default2dTheme = {
    ...
    pubPointImg: {
        ...
        "21004": System.imgPath+"/location.png"
    }
}
var default3dTheme = {
    name: "test", //theme's name
    ...
    function init(params) {
        ...
        if(params != undefined){
            ...
            if (params.hasOwnProperty("mapDiv")) {
              ...
              _mapDiv.style.width = window.screen.width+'px'
              _mapDiv.style.height = window.screen.height+'px'
                _fullScreen = false;
            }
...
IndoorMap.getUI = function(indoorMap){
    ...
    _uiRoot = document.getElementById('floorsUI');
    ...
    for(var i = 0; i < floors.length; i++){
        (function(arg){
            ...
            li.className = 'floorsUI-item';
            li.setAttribute('id','item'+i)
            if(i === 0){
              li.style.color = '#628CFD'
            }
            // li.setAttribute('onclick','mapChange('+i+')');
            text = document.createTextNode(floors[arg].Name);
            li.appendChild(text);
            li.onclick = function () {
                _indoorMap.showFloor(floors[arg]._id);
                for(var j = 0;j<floors.length; j++){
                  if(j !== (floors[arg]._id-1)){
                    document.getElementById('item'+j).style.color = '#333333'
                  }else{
                    document.getElementById('item'+j).style.color = '#628CFD'
                  }
                }
            }
            _uiRoot.appendChild(li);
